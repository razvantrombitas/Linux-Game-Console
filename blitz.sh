...
function rep { 
	local i
	for i in $(seq 1 1 $2) # generate the sequence 
	do printf "%s" "$1"
	done
}
...
function motor {
    ...
    function readkey {
        ...
        read k
        if [[ $k = q ]]; then clear;stty echo;exit; fi
        if ((INICIO==1)); then 
            ...
            if ((B>0));then 
                for ((M=1;M<=4;M++));do 
                    ...
                    if ((B==1 && BY0+BY<=0)); then 
                        ...
                        printf "\e[$((Y0-2));$((X0+BX*2))H  \e[$((Y0-1));$((X0+BX*2))H  "
                        B=0;((vt=A[BX]))
